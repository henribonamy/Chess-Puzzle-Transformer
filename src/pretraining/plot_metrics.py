...
def plot_metrics(metrics_file="outputs/training_metrics.json"):
    ...
    if validity_history or uniqueness_history or novelty_history:
        ...
        if validity_history:
            ...
            plt.plot(iterations_v, validity_vals, label='Validity', marker='o', markersize=3)
        if uniqueness_history:
            iterations_u, uniqueness_vals = zip(*uniqueness_history)
            plt.plot(iterations_u, uniqueness_vals, label='Uniqueness', marker='s', markersize=3)
        if novelty_history:
            iterations_n, novelty_vals = zip(*novelty_history)
            plt.plot(iterations_n, novelty_vals, label='Novelty', marker='^', markersize=3)
        plt.xlabel('Iteration')
        plt.ylabel('Score (%)')
